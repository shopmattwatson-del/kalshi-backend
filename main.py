...
app = FastAPI(title="KalshiPRO Backend", version="2.0.0")
...
def get_private_key():
    ...
    try:
        ...
        raise HTTPException(500, "Failed to load private key - check KALSHI_PRIVATE_KEY format")

# ── Signing ──────────────────────────────────────────────────────────────────

def make_signature(method: str, path: str, timestamp_ms: str) -> str:
    """
    Kalshi signature = RSA-PSS-SHA256( timestamp_ms + METHOD + /trade-api/v2/endpoint )
    Query params are stripped before signing.
    """
    path_no_query = path.split("?")[0]
    # Ensure full path includes /trade-api/v2 prefix
